...
| 月薪 | `salary` | number | 初始12000，可加薪 |
| 生活费 | `living_cost` | number | 月初扣除，初始6000 |
...
每月固定扣除：
  money -= living_cost（生活费，初始6000）
  money += salary（月薪，初始12000）
```
...
```
月初：发工资(+12000) → 扣生活费(-6000) → 自然恢复 → 模型解锁 → 月份大事件
...
| 支出项 | 金额 | 频率 | 说明 |
|:---|:---|:---|:---|
| 生活费 | 6000（`living_cost`） | 每月 | 月初自动扣除 |
