...
function buildDeltaHtml(deltas, orderedKeys) {
    if (!deltas) return '';
    return orderedKeys
        .filter(key => Object.prototype.hasOwnProperty.call(deltas, key))
...
let overlayPauseTokenSeq = 0;
const overlayPauseStack = [];

function acquireOverlayPause(overlayId) {
    const token = `${overlayId}-${++overlayPauseTokenSeq}`;
    overlayPauseStack.push({ overlayId, token });
    game.pause();
    return token;
}

function releaseOverlayPause(token) {
    const idx = overlayPauseStack.findIndex(entry => entry.token === token);
    if (idx === -1) return;
    overlayPauseStack.splice(idx, 1);
    game.resume();
}
...
function openOverlay(overlayId) {
    ...
    if (!overlay) return;
    if (!overlay.dataset.pauseToken) {
        overlay.dataset.pauseToken = acquireOverlayPause(overlayId);
    }
...
function closeOverlay(overlayId) {
    ...
    if (!overlay.dataset.pauseToken) return;
    const token = overlay.dataset.pauseToken;
    delete overlay.dataset.pauseToken;
    releaseOverlayPause(token);
}
...
game.onChoiceResult = ({ text, deltas }) => {
    const ordered = ['hp', 'brain', 'money', 'bossSatisfy', 'shaoye_rel', 'yimin_rel', 'gf_rel'];
    const deltaHtml = buildDeltaHtml(deltas, ordered);
    const html = deltaHtml ? `→ ${text} ${deltaHtml}` : `→ ${text}`;
